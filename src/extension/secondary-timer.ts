...
import { secondaryTimerReplicant as timerRep } from './util/replicants';
import { TaggedLogger } from './util/tagged-logger';

const nodecg = get();
let timer: livesplitCore.Timer;
const logger = new TaggedLogger('SecondaryTimer');
...
nodecg.listenFor('secondaryTimerStart', (data, ack) => {
  startTimer(true)
    .then(() => processAck(ack, null))
    .catch((err) => processAck(ack, err));
});
nodecg.listenFor('secondaryTimerPause', (data, ack) => {
  pauseTimer()
    .then(() => processAck(ack, null))
    .catch((err) => processAck(ack, err));
});
nodecg.listenFor('secondaryTimerReset', (force, ack) => {
  resetTimer(force)
    .then(() => processAck(ack, null))
    .catch((err) => processAck(ack, err));
});
nodecg.listenFor('secondaryTimerFinish', (data, ack) => {
  stopTimer()
    .then(() => processAck(ack, null))
    .catch((err) => processAck(ack, err));
});
