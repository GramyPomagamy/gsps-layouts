...
const channelNameToId = Object.assign({}, ...Object.entries(channelIdToName).map(([k, v]) => ({ [v]: k })))
const mixerSignalLevels = nodecg.Replicant<{ [key in Channel]: number }>('mixerSignalLevels', { defaultValue: {
  "": -Infinity,
  "H1": -Infinity,
  "H2": -Infinity,
  "H3": -Infinity,
...
const mixerThresholdLevels = nodecg.Replicant<{ [key in Channel]: number }>('mixerThresholdLevels', { defaultValue: {
  "": -Infinity,
  "H1": -30,
  "H2": -30,
  "H3": -30,
...
mixerThresholdLevels;  // has to be used o__O
