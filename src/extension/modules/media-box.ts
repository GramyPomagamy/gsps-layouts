...
export async function setup({ nodecg }: ModuleParams<object>): Promise<void> {
  ...
  const mediaBoxBreakAssets = nodecg.Replicant<Asset[]>(
    "assets:media-box-break",
  );
  ...
  const showNextBreakMediaBoxItem = () => {
    ...
    if (mediaBoxBreakAssets.value && mediaBoxBreakAssets.value.length > 0) {
      ...
      if (asset && asset.ext) {
        ...
        currentMediaBoxBreakItem.value = { asset: structuredClone(asset) };
        return;
      }
    } else {
      // if no assets found, try again in one second
