...
const TOTAL_URL = 'https://donacje.gsps.pl/17?json';
const totalLog = new (nodecg() as NodeCG).Logger(`${nodecg().bundleName}:total`);
const donationSocketUrl = (nodecg().bundleConfig as Configschema).donationSocketUrl;
...
if (donationSocketUrl) {
	...
	totalLog.warn(`cfg/${nodecg().bundleName}.json is missing the "donationSocketUrl" property.` +
 ...
 * @returns {undefined}
 */
function manuallyUpdateTotal(silent: boolean = false, cb = function (error?: any, updated?: any) {}) {
