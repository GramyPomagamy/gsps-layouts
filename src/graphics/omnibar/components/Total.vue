...
export default class OmnibarTotal extends Vue {
    ...
    mounted() {
        const updateHandler = () => {
            document.getElementById('totalAmount')!.innerHTML = this.$data.total.raw.toLocaleString('en-US', {
                maximumFractionDigits: 0,
				minimumFractionDigits: 0
            });
        }
        totalRep.on('change', (newVal) => {
            gsap.to(this.$data.total, {duration: 3, raw: newVal.raw, roundProps:"raw", onUpdate: updateHandler, ease: "power4"})
        })
    }
}
</script>
