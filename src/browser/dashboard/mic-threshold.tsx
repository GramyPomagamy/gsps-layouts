...
export const MicThreshold = () => {
  const [mixerThresholdLevels, setMixerThresholdLevels] = useReplicant<{
    [key in keyof typeof channelNameToId]: number;
  }>(
    'mixerThresholdLevels',
    Object.keys(channelNameToId).reduce(
      (acc, name) => {
        acc[name] = -30;
        return acc;
      },
  ...
  const [localThresholdLevels, setLocalThresholdLevels] = useState<{
    [key in keyof typeof channelNameToId]: number;
  }>(
    Object.keys(channelNameToId).reduce(
      (acc, name) => {
            ...
            return (
              <React.Fragment key={key}>
                ...
                <Box
                  ...
                  <Typography
                    variant="body1"
                    sx={{ fontWeight: '500', minWidth: '45px', textAlign: 'right' }}>
                    {value.toFixed(1)}
                  </Typography>
        ...
        <Box
          ...
          {isSaving ? (
            ...
            <>
              ...
              <Typography
                variant="body2"
                sx={{ color: 'rgba(85, 170, 85, 1)', fontWeight: 'bold' }}>
                Wszystko zapisano
              </Typography>
            </>
          )}
