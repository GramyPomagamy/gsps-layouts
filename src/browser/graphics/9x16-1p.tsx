...
import layoutBg from './img/layouts/9x16-1p.png';
import layoutBgWithDonationBar from './img/layouts/9x16-1p-donationbar.png';
...
const Info = styled.div<{ showDonationBar: boolean }>`
  width: ${(props) => (props.showDonationBar ? '680px' : '668px')};
  height: ${(props) => (props.showDonationBar ? '474px' : '488px')};
  position: fixed;
  bottom: ${(props) => (props.showDonationBar ? '48px' : '0px')};
  text-align: center;
  margin: 0;
  display: flex;
...
const MediaBoxDiv = styled.div<{ showDonationBar: boolean }>`
  right: 0px;
  position: fixed;
  bottom: ${(props) => (props.showDonationBar ? '48px' : '0px')};
  width: ${(props) => (props.showDonationBar ? '680px' : '668px')};
  ...
  return (
    <ThemeProvider>
      <LayoutContainer showDonationBar={showDonationBar}>
        ...
        <MediaBoxDiv showDonationBar={showDonationBar}>
          <MediaBox />
        </MediaBoxDiv>
        {showDonationBar && (
          <Donations>
