...
import layoutBg from './img/layouts/5x3-1p.png';
import layoutBgWithDonationBar from './img/layouts/5x3-1p-donationbar.png';
...
const LeftSide = styled.div<{ showDonationBar: boolean }>`
  position: fixed;
  width: ${(props) => (props.showDonationBar ? '641px' : '568px')};
...
const BottomRight = styled.div<{ showDonationBar: boolean }>`
  position: fixed;
  width: ${(props) => (props.showDonationBar ? '1275px' : '1347px')};
  height: 213px;
  left: ${(props) => (props.showDonationBar ? '645px' : '573px')};
